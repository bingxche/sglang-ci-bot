...
def _check_ci_with_agent(pr_number: int, repo_path) -> str:
    ...
    prompt = f"""Check all CI status for PR #{pr_number} in sgl-project/sglang.
Use the GitHub API (auth token in $GH_PAT or $BOT_PAT) to:
1. Get the PR's head commit SHA
2. List all workflow runs for that SHA
3. For any failed jobs, download the logs and identify the errors
4. Fetch the PR diff and determine if each failure is related to the PR changes

Format your output as a markdown report:
## CI Status for PR #{pr_number}

Start with a one-line summary: how many passed, failed, pending.

Then a table:
| Workflow | Job | Error | Related to PR? | Explanation |
|----------|-----|-------|----------------|-------------|

For the "Related to PR?" column, use:
- "Likely" if the error involves code paths touched by the PR
- "Possibly" if it might be related
- "Unlikely" if it's unrelated/infra/flaky

After the table, briefly explain your reasoning for each verdict.
